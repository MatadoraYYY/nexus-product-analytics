...
aasync function getJson(path: string) {
...
function App() {
  ...
  const cards = useMemo(() => d ? [
    ['DAU', fmt(d.dau)],
    ['MAU', fmt(d.mau)],
    ['Stickiness', fmt(d.stickiness, true)],
    ['Activation', fmt(d.activation_rate, true)],
    ['D30 Retention', fmt(d.d30_retention, true)],
    ['MRR', `$${fmt(d.mrr)}`],
    ['Churn', fmt(d.churn, true)],
    ['LTV:CAC', fmt(d.ltv_cac)],
  ] : [], [d]);

  if (error && !data) {
    return <main className="shell"><section className="state"><div className="brand">NEXUS</div><h1>Product Intelligence</h1><p>{error}</p><span>Повторяем подключение автоматически…</span></section></main>;
  }

  if (!data || !d) {
    return <main className="shell"><section className="state"><div className="spinner" /><div className="brand">NEXUS</div><h1>Загрузка аналитики</h1><p>Подключаемся к аналитическому API и готовим показатели.</p></section></main>;
  }
  ...
  return <main className="shell">
    <header className="topbar">
      <div>
        ...
        <p className="subtitle">Продуктовая аналитика в одном окне</p>
      </div>
      <div className="statusBlock">
        <span className="status"><i /> API ONLINE</span>
        <span className="period">Период: {d.period}</span>
      </div>
    </header>
