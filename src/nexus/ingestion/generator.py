...
EVENTS = ["signup", "login", "project_created", "task_created", "team_invited", "feature_used", "checkout_started", "subscription_started", "subscription_cancelled"]
...
def generate_users(cfg: GeneratorConfig) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        "user_id": _uids(n, rng), "registration_timestamp": pd.to_datetime(ts, utc=True), "registration_date": registration_date,
        "country": rng.choice(COUNTRIES, n, p=[.25, .20, .15, .10, .10, .20]), "device": rng.choice(DEVICES, n, p=[.55, .25, .20]),
        "acquisition_channel": rng.choice(CHANNELS, n, p=[.30, .20, .15, .15, .10, .10]), "plan_at_signup": rng.choice(PLANS, n, p=[.75, .20, .05]), "dataset_version": "1.0.0"
    })
...
def generate_events(users: pd.DataFrame, cfg: GeneratorConfig) -> pd.DataFrame:
    ...
    for r in users.itertuples(index=False):
        ...
        if r.plan_at_signup in ("pro", "team") or rng.random() < .04:
            ...
            rows.append((r.user_id, subday, "checkout_started", None))
