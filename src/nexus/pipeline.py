...
from src.nexus.ingestion.generator import (
    GeneratorConfig,
    generate_events,
    generate_financials,
    generate_users,
...
def run():
    ...
    tables = {
        ...
        "mart_daily_kpis": marts["daily"],
        "mart_activation": marts["activation"],
        "mart_funnel": marts["funnel"],
        "mart_retention": marts["retention"],
        "mart_revenue_monthly": marts["revenue"],
        "mart_customer_economics": marts["economics"],
        "mart_feature_usage": marts["features"],
    }
    for name, df in tables.items():
        con.register(name + "_df", df)
